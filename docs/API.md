...
| 항목      | 값                        |
|-----------|--------------------------|
| Base URL  | `http://yhjang1.shop:3000` |
| 인코딩    | UTF-8                     |
| 응답 형식 | JSON                      |
...
| 상태코드 | 조건              | 응답 본문                              |
|----------|-------------------|----------------------------------------|
| 400      | `name` 파라미터 없음 | `no data of name : {name}`          |
| 500      | 사용자를 찾지 못함  | `query no result`                    |
| 500      | DB 쿼리 오류      | `query error : {err}`                  |
